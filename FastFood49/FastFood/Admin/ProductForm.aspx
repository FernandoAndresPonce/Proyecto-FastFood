...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
       ...
       <script>
           function ImagePreview(input) {
               var imgElement = $('#<%=imgForm.ClientID%>'); // Obtener el elemento de imagen

               if (input.files && input.files[0]) {
                   if (input.files[0].size > 5242880) { // 5MB en bytes
                       alert("The file size exceeds the allowed limit of 5 MB.");
                       // Limpiar el campo de entrada
                       $(input).val(''); // Limpia el input
                       imgElement.prop('src', '../TemplateFiles/images/placeholder.jpg'); // Limpiar la imagen
                       return;
                   ...
                   imgElement.prop('src', '../TemplateFiles/images/perfil.png'); // Establecer la imagen por defecto si no hay archivo
               }
           }
</script>
