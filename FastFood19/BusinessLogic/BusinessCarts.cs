        ...
        public List<CartD> ListCart(UsersD userId)
        {
            ...
            try
            {
                ...
                while (data.Read.Read())
                {
                    ...
                    auxCart.ProductId.Name = (string)data.Read["Name"];

                    //if (!(data.Read["Name"] is DBNull))
                    //{
                    //    auxCart.Name = new ProductD();
                    //    auxCart.Name.Name = (string)data.Read["Name"];
                    //}
                    if (!(data.Read["ImageUrl"] is DBNull))
                    {
                        auxCart.ImageUrl = new ProductD();
                        auxCart.ImageUrl.ImageUrl = (string)data.Read["ImageUrl"];
                    }

                    if (!(data.Read["PrdQty"] is DBNull))
                    {
                        auxCart.PrdQty = new ProductD();
                        auxCart.PrdQty.Quantity = (int)data.Read["PrdQty"];
                    }
                    if (!(data.Read["Price"] is DBNull))
                    {
                        auxCart.Price = new ProductD();
                        auxCart.Price.Price = (decimal)data.Read["Price"];
                    }
